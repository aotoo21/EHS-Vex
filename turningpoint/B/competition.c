#pragma config(Motor,  port2,           leftbackwheel, tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port3,           leftfrontwheel, tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port4,           rightbackwheel, tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port5,           rightfrontwheel, tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port6,           leftarm,       tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port7,           rightarm,      tmotorVex393_MC29, openLoop)
//*!!Code automatically generated by 'ROBOTC' configuration wizard               !!*//

/*---------------------------------------------------------------------------*/
/*                                                                           */
/*        Description: Competition template for VEX EDR                      */
/*                                                                           */
/*---------------------------------------------------------------------------*/

// This code is for the VEX cortex platform
#pragma platform(VEX2)

// Select Download method as "competition"
#pragma competitionControl(Competition)

//Main competition background code...do not modify!
#include "Vex_Competition_Includes.c"

/*---------------------------------------------------------------------------*/
/*                          Pre-Autonomous Functions                         */
/*                                                                           */
/*  You may want to perform some actions before the competition starts.      */
/*  Do them in the following function.  You must return from this function   */
/*  or the autonomous and usercontrol tasks will not be started.  This       */
/*  function is only called once after the cortex has been powered on and    */
/*  not every time that the robot is disabled.                               */
/*---------------------------------------------------------------------------*/

void pre_auton()
{
  // Set bStopTasksBetweenModes to false if you want to keep user created tasks
  // running between Autonomous and Driver controlled modes. You will need to
  // manage all user created tasks if set to false.
  bStopTasksBetweenModes = true;

	// Set bDisplayCompetitionStatusOnLcd to false if you don't want the LCD
	// used by the competition include file, for example, you might want
	// to display your team name on the LCD in this function.
	// bDisplayCompetitionStatusOnLcd = false;

  // All activities that occur before the competition starts
  // Example: clearing encoders, setting servo positions, ...
}

/*---------------------------------------------------------------------------*/
/*                                                                           */
/*                              Autonomous Task                              */
/*                                                                           */
/*  This task is used to control your robot during the autonomous phase of   */
/*  a VEX Competition.                                                       */
/*                                                                           */
/*  You must modify the code to add your own robot specific commands here.   */
/*---------------------------------------------------------------------------*/

task autonomous()
{

  // ..........................................................................
  // Insert user code here.
  // ..........................................................................

  // Remove this function call once you have "real" code.
  AutonomousCodePlaceholderForTesting();
}

/*---------------------------------------------------------------------------*/
/*                                                                           */
/*                              User Control Task                            */
/*                                                                           */
/*  This task is used to control your robot during the user control phase of */
/*  a VEX Competition.                                                       */
/*                                                                           */
/*  You must modify the code to add your own robot specific commands here.   */
/*---------------------------------------------------------------------------*/

task usercontrol()
{
  // User control code here, inside the loop

  while (true)
  {

    // This is the main execution loop for the user control program.
    // Each time through the loop your program should update motor + servo
    // values based on feedback from the joysticks.

    // ........................................................................
    // Insert user code here. This is where you use the joystick values to
    // update your motors, etc.
    // ........................................................................

    // Remove this function call once you have "real" code.

    motor[leftbackwheel] = vexRT[Ch3];
    motor[leftfrontwheel] = vexRT[Ch3];
    motor[rightbackwheel] = vexRT[Ch2];
    motor[rightfrontwheel] = vexRT[Ch2];
		if (vexRT[Btn5U])
		{
			motor[leftarm] = 255;
			motor[rightarm] = 255;
		}
		else if (vexRT[Btn5D]) {
			motor[leftarm] = -255;
			motor[rightarm] = -255;
		}
		else {
			motor[leftarm] = 0;
			motor[rightarm] = 0;
		}
		if (vexRT[Btn8R])
		{
			motor[leftbackwheel]=255;
			motor[leftfrontwheel]=255;
			motor[rightbackwheel]=255;
			motor[rightfrontwheel]=255;
			wait1Msec(2960);
			motor[leftbackwheel]=0;
			motor[leftfrontwheel]=0;
			motor[rightbackwheel]=0;
			motor[rightfrontwheel]=0;
			motor[rightarm]=255;
			motor[leftarm]=255;
			wait1Msec(1590);
			motor[leftarm]=0;
			motor[rightarm]=0;
			motor[leftbackwheel]=255;
			motor[leftfrontwheel]=255;
			motor[rightbackwheel]=255;
			motor[rightfrontwheel]=255;
			wait1Msec(1800);
			motor[leftbackwheel]=0;
			motor[leftfrontwheel]=0;
			motor[rightbackwheel]=0;
			motor[rightfrontwheel]=0;
			/*wait1Msec(50);
			motor[leftbackwheel]=-255;
			motor[leftfrontwheel]=-255;
			motor[rightbackwheel]=255;
			motor[rightfrontwheel]=255;
			wait1Msec(910);
			motor[leftbackwheel]=0;
			motor[leftfrontwheel]=0;
			motor[rightbackwheel]=0;
			motor[rightfrontwheel]=0;
			wait1Msec(50);
			motor[leftbackwheel]=255;
			motor[leftfrontwheel]=255;
			motor[rightbackwheel]=255;
			motor[rightfrontwheel]=255;
			wait1Msec(3740);
			motor[leftbackwheel]=0;
			motor[leftfrontwheel]=0;
			motor[rightbackwheel]=0;
			motor[rightfrontwheel]=0;*/
		}
		if (vexRT[Btn8L])
		{
			motor[leftfrontwheel]=255;
			motor[leftbackwheel]=255;
			motor[rightfrontwheel]=255;
			motor[rightbackwheel]=255;
			wait1Msec(1140);
			motor[leftarm]=255;
			motor[rightarm]=255;
			wait1Msec(2400);
			motor[rightfrontwheel]=0;
			motor[rightbackwheel]=0;
			motor[leftarm]=0;
			motor[rightarm]=0;
			motor[leftfrontwheel]=0;
			motor[leftbackwheel]=0;
			wait1Msec(70);
			motor[leftfrontwheel]=255;
			motor[leftbackwheel]=255;
			motor[rightfrontwheel]=-255;
			motor[rightbackwheel]=-255;
			wait1Msec(1100);
			motor[leftfrontwheel]=0;
			motor[leftbackwheel]=0;
			motor[rightfrontwheel]=0;
			motor[rightbackwheel]=0;
			wait1Msec(70);
			motor[leftfrontwheel]=255;
			motor[leftbackwheel]=255;
			motor[rightfrontwheel]=255;
			motor[rightbackwheel]=255;
			wait1Msec(3970);
		}
		if (vexRT[Btn7R])
		{
			motor[leftfrontwheel]=255;
			motor[leftbackwheel]=255;
			motor[rightfrontwheel]=255;
			motor[rightbackwheel]=255;
			wait1Msec(1140);
			motor[leftarm]=255;
			motor[rightarm]=255;
			wait1Msec(2400);
			motor[rightfrontwheel]=0;
			motor[rightbackwheel]=0;
			motor[leftarm]=0;
			motor[rightarm]=0;
			motor[leftfrontwheel]=0;
			motor[leftbackwheel]=0;
			wait1Msec(70);
			motor[leftfrontwheel]=-255;
			motor[leftbackwheel]=-255;
			motor[rightfrontwheel]=255;
			motor[rightbackwheel]=255;
			wait1Msec(1100);
			motor[leftfrontwheel]=0;
			motor[leftbackwheel]=0;
			motor[rightfrontwheel]=0;
			motor[rightbackwheel]=0;/*
			wait1Msec(70);
			motor[leftfrontwheel]=255;
			motor[leftbackwheel]=255;
			motor[rightfrontwheel]=255;
			motor[rightbackwheel]=255;
			wait1Msec(3970);*/
		}
		if (vexRT[Btn7L])
		{
			motor[leftbackwheel]=255;
			motor[leftfrontwheel]=255;
			motor[rightbackwheel]=255;
			motor[rightfrontwheel]=255;
			wait1Msec(2960);
			motor[leftbackwheel]=0;
			motor[leftfrontwheel]=0;
			motor[rightbackwheel]=0;
			motor[rightfrontwheel]=0;
			motor[rightarm]=255;
			motor[leftarm]=255;
			wait1Msec(1590);
			motor[leftarm]=0;
			motor[rightarm]=0;
			motor[leftbackwheel]=255;
			motor[leftfrontwheel]=255;
			motor[rightbackwheel]=255;
			motor[rightfrontwheel]=255;
			wait1Msec(1800);
			motor[leftbackwheel]=0;
			motor[leftfrontwheel]=0;
			motor[rightbackwheel]=0;
			motor[rightfrontwheel]=0;
			wait1Msec(50);
			motor[leftbackwheel]=255;
			motor[leftfrontwheel]=255;
			motor[rightbackwheel]=-255;
			motor[rightfrontwheel]=-255;
			wait1Msec(910);
			motor[leftbackwheel]=0;
			motor[leftfrontwheel]=0;
			motor[rightbackwheel]=0;
			motor[rightfrontwheel]=0;
			wait1Msec(50);
			motor[leftbackwheel]=255;
			motor[leftfrontwheel]=255;
			motor[rightbackwheel]=255;
			motor[rightfrontwheel]=255;
			wait1Msec(3740);
			motor[leftbackwheel]=0;
			motor[leftfrontwheel]=0;
			motor[rightbackwheel]=0;
			motor[rightfrontwheel]=0;
		}
		while (vexRT[Btn6U])
		{
			motor[leftfrontwheel]=255;
			motor[leftbackwheel]=255;
			motor[rightfrontwheel]=255;
			motor[rightbackwheel]=255;
		}
		while (vexRT[Btn6D])
		{
			motor[leftfrontwheel]=-255;
			motor[leftbackwheel]=-255;
			motor[rightfrontwheel]=-255;
			motor[rightbackwheel]=-255;
		}
		/*
		blue side left 2960 1590 1800 870 3740
		if (vexRT[Btn5U]==0)
		{
			motor[leftarm] = 0;
			motor[rightarm] = 0;
		}	`
		if (vexRT[Btn6U]==1)
		{
			motor[leftarm] = -255;
			motor[rightarm] = -255;
		if (vexRT[Btn6U]==0)
		{
			motor[leftarm] = 0;
			motor[rightarm] = 0;
		}
		}
		*/
  }
}
